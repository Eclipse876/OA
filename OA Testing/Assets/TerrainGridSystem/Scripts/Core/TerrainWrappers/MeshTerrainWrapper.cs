...
namespace TGS {
    ...
    public class MeshTerrainWrapper : ITerrainWrapper {
        ...
        public MeshTerrainWrapper(GameObject gameObject, string prefix, LayerMask layerMask, bool searchGlobal, int heightmapWidth, int heightmapHeight) {
            ...
            // check if there's a sphere collider with non-uniform scale
            if (gameObject.transform.localScale != Misc.Vector3one) {
        ...
        void GetRenderers() {
            ...
            for (int k = 0; k < renderers.Length; k++) {
                ...
                if (r != null) {
                    ...
                    bool matchesPrefix = !usesPrefix || r.name.StartsWith(_prefix, System.StringComparison.InvariantCultureIgnoreCase);
            ...
            for (int k = 0; k < terrains.Length; k++) {
                ...
                if (t != null) {
                    ...
                    bool matchesPrefix = !usesPrefix || t.name.StartsWith(_prefix, System.StringComparison.InvariantCultureIgnoreCase);
                    if (matchesLayer && matchesPrefix) {
                        RendererInfo ri = new RendererInfo();
                        rr.Add(ri);
                        ri.terrain = t;
        ...
        void ComputeDepthAndNormals() {

            TGSURPCameraSetup urpHelper = null;

            lossyScale = _gameObject.transform.lossyScale;
            GameObject camObject;
            if (TGSHDRPCameraSetup.usesHDRP) {
                camObject = Object.Instantiate(Resources.Load<GameObject>("Prefabs/DepthCamHDRP"));
            } else {
                camObject = Object.Instantiate(Resources.Load<GameObject>("Prefabs/DepthCam"));
                urpHelper = camObject.GetComponent<TGSURPCameraSetup>();
            }
            ...
            float farClipPlane = top.y - boundsMin.y;
            cam.farClipPlane = farClipPlane;

            if (urpHelper != null) urpHelper.PreShot();
            ...
            if (urpHelper != null) urpHelper.PostShot();
        ...
        void BackupMaterials() {
            ...
            for (int k = 0; k < rrCount; k++) {
                ...
                if (ri.renderer != null) {
                    ...
                    ri.enabled = ri.renderer.enabled;
                    if (!ri.renderer.enabled) {
                    ...
                    ri.drawTreesAndFoliage = ri.terrain.drawTreesAndFoliage;
                    if (!ri.terrain.enabled) {
                        ri.terrain.enabled = true;
                    }
                }
            }
        }
        ...
        void SetShader(string shaderName) {
            ...
            for (int k = 0; k < rrCount; k++) {
                ...
                if (ri.renderer != null) {
                    ...
                    ri.terrain.drawTreesAndFoliage = false;
                }
            }
        }
        ...
        void RestoreMaterials() {
            ...
            for (int k = 0; k < rrCount; k++) {
                ...
                if (ri.renderer != null) {
                    ...
                    ri.terrain.drawTreesAndFoliage = ri.drawTreesAndFoliage;
                    if (!ri.enabled) {
                        ri.terrain.enabled = true;
                    }
                }
        ...
        public int GetAlphamapIndex(Vector3 position, int terrainTextureCount) {
            Debug.LogError("GetAlphaMapIndex can only be used on Unity terrain, not mesh based terrains.");
            return 0;
        }

        public void SetAlphamapOpacity(Vector3 position, int splatmapIndex, float opacity = 1f, bool hideOtherLayers = true) {
            Debug.LogError("SetAlphamapOpacity can only be used on Unity terrain, not mesh based terrains.");
            return;
        }
        ...
        public float[,,] GetAlphamap(Vector3 position) {
            Debug.LogError("GetAlphamap can only be used on Unity terrain, not mesh based terrains.");
            return null;
        }

        public Vector2Int WorldToAlphamapCoordinates(Vector3 position) {
            Debug.LogError("WorldToAlphamapCoordinates can only be used on Unity terrain, not mesh based terrains.");
            return default;
        }

        public Vector3 AlphamapToWorldCoordinates(Vector2Int coords) {
            Debug.LogError("AlphamapToWorldCoordinates can only be used on Unity terrain, not mesh based terrains.");
            return default;
        }
