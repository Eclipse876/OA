...
namespace OA.Presentation.Debug
{
    ...
    public sealed class PathfindingSandboxController : MonoBehaviour
    {
        ...
        [Header("Turn-Aware Course Selection")]
        [SerializeField, Min(0.01f)] private float intermediateWaypointRadiusWorld = 0.55f;
        [SerializeField, Range(0f, 1f)] private float plannedTurnSmoothingStrength = 1f;
        [SerializeField, Range(1, 3)] private int plannedTurnShapeAttempts = 3;
        ...
        private readonly List<ShipRouteGate> routeGates = new List<ShipRouteGate>(32);
        ...
        private int nextRouteGateIndex;
        private Vector2 previousGateCheckPosition;
        private bool hasPreviousGateCheckPosition;
        ...
        private bool TryRouteThroughWaypoints(bool silent)
        {
            ...
            float bestTurnSpeedScale = 1f;
            float bestSmoothingStrength = 0f;
            bool stopSearching = false;

            // Compare every maneuver-speed tier. A faster legal turn is not
            // preferable when it makes a large wrong-way loop after crossing
            // an intermediate order and a slower turn exits cleanly.
            for (int maneuverAttempt = 0;
                 maneuverAttempt < maneuverAttempts && !stopSearching;
                 maneuverAttempt++)
            {
                bool foundValidAtThisManeuverSpeed = false;
                     ...
                     clearanceAttempt++)
                {
                    ...
                    int shapeAttempts = Mathf.Clamp(plannedTurnShapeAttempts, 1, 3);

                    for (int shapeAttempt = 0;
                         shapeAttempt < shapeAttempts && !stopSearching;
                         shapeAttempt++)
                    {
                        float smoothingStrength = GetCourseSmoothingStrength(
                            shapeAttempt,
                            shapeAttempts);
                        ...
                        if (!candidateRoute.IsValid)
                        {
                            ...
                            lastFailureGateIndex = candidateRoute.FailureGateIndex;

                            // Alternative curve shape, speed, or clearance can
                            // fix clipping or missed fly-by gates. Data/course
                            // construction failures cannot.
                            if (candidateRoute.FailureReason !=
                                    ShipRouteFailureReason.PredictedTrackBlocked &&
                                candidateRoute.FailureReason !=
                        ...
                        foundValidAtThisManeuverSpeed = true;

                        if (candidateRoute.ManeuverScore < bestScore)
                        {
                            bestScore = candidateRoute.ManeuverScore;
                            bestClearance = extraClearance;
                            bestTurnSpeedScale = constrainedTurnSpeedScale;
                            bestSmoothingStrength = smoothingStrength;
                            bestCandidateRoute.CopyFrom(candidateRoute);
                        }
                    }

                    // Wider clearance is a recovery tool, not an alternate
                    // scenic-route generator. Once this least restrictive mask
                    // has executable shape options, keep its best one.
                    if (foundValidAtThisManeuverSpeed)
                    {
                        break;
                    }
                }
            ...
            {
                activeRoute.CopyFrom(bestCandidateRoute);
                activeRouteVisibleSampleIndex = 0;
                nextRouteGateIndex = 0;
                previousGateCheckPosition = GetShipPosition();
                hasPreviousGateCheckPosition = true;
                ...
                if (!silent)
                {
                    LogStatus(
                        ...
                        $"PlanningClearance={bestClearance:F2}. " +
                        $"TightTurnSpeedScale={bestTurnSpeedScale:F2}. " +
                        $"CurveStrength={bestSmoothingStrength:F2}. " +
                        $"Reversals={activeRoute.HeadingReversalCount}. " +
                        $"GatePenalty={activeRoute.GateDeparturePenalty:F2}. " +
                        $"MaxBacktrack={activeRoute.MaximumGateBacktrackWorld:F2}. " +
                        $"Score={bestScore:F2}.");
                }
        ...
        // Builds physical route-order gates. Intermediate orders allow a fly-by;
        // the final order remains the precise stopped destination.
        private void BuildRouteGates()
        {
            ...
            {
                bool finalWaypoint = i == routeWaypoints.Count - 1;
                float radius = finalWaypoint
                    ? shipAgent.WaypointReachDistance
                    : intermediateWaypointRadiusWorld;

                routeGates.Add(new ShipRouteGate(
                    routeWaypoints[i].World,
                    radius,
                    finalWaypoint));
            }
        }

        // Prefer the fully anticipated turn, then a milder curve, then the
        // unshaped fallback so shaping cannot eliminate an executable order.
        private float GetCourseSmoothingStrength(int attempt, int attempts)
        {
            if (attempts <= 1 || attempt <= 0)
            {
                return plannedTurnSmoothingStrength;
            }
            ...
            return plannedTurnSmoothingStrength * 0.55f;
        }
            ...
            int gateIndex)
        {
            switch (reason)
            {
                ...
                case ShipRouteFailureReason.MissedRequiredWaypoint:
                    if (gateIndex >= 0)
                    {
                        return
                            $"A safe physical route was found, but its turning " +
                            $"track does not pass through ordered waypoint " +
                            $"{gateIndex + 1}.";
                    }

                    return
                        "A safe physical route was found, but its turning track " +
                        "does not pass through each ordered waypoint gate.";
        ...
        // Intermediate waypoint markers retire only when the physical live track
        // crosses their accepted gate, matching the prediction validation rule.
        private void RetirePassedWaypoints()
        {
            if (!shipAgent.HasPath() ||
                routeWaypoints.Count <= 1 ||
                activeRoute.Gates.Count == 0)
            {
                return;
            }

            Vector2 currentPosition = GetShipPosition();

            if (!hasPreviousGateCheckPosition)
            {
                previousGateCheckPosition = currentPosition;
                hasPreviousGateCheckPosition = true;
            }
            ...
            while (routeWaypoints.Count > 1 &&
                   nextRouteGateIndex < activeRoute.Gates.Count - 1)
            {
                ShipRouteGate gate = activeRoute.Gates[nextRouteGateIndex];

                if (DistanceFromSegmentToPoint(
                        previousGateCheckPosition,
                        currentPosition,
                        gate.Position) > gate.RadiusWorld)
                {
                    break;
                }
